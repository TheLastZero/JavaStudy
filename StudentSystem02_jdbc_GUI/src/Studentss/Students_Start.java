...
public class Students_Start extends JFrame implements ActionListener{
	...
	JLabel bq1;//标签
	JTextField wbk1;//文本框
	JButton an1,an11,an2,an3,an4,an5;//5个按钮
	JTable bg1;//表格
	JScrollPane gd1;//滚动条
	...
	Students_Start(){//构造方法
		...
		an5=new JButton("查看学生借书信息");//删除按钮
		an5.addActionListener(this);
		an5.setActionCommand("stuInfo");
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getActionCommand().equals("find")){//查找
			...
			book_find sf=new book_find(sql);
		...
		if(e.getActionCommand().equals("modify")){//修改
			...
			book_modify sf6=new book_modify(this,"修改书籍信息",true,ii);
		...
		if(e.getActionCommand().equals("add")){//添加
			...
			book_add a=new book_add(this,"添加书籍信息",true);
			//为了方便添加之后看到所有学生信息，这里再new一个
			book_find sf2=new book_find(); 
			bg1.setModel(sf2);	
		...
		if(e.getActionCommand().equals("delete")){//删除
			...
			try{
				...
				//写SQL语句
				String s=("delete bookInfo where bookId=?");
			...
			book_find sf5=new book_find();
			bg1.setModel(sf5);
			System.out.println("delete");
		}
		
		if(e.getActionCommand().equals("stuInfo")){//查看学生信息
